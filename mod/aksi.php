<?php 
	...
	$cek_tgl 	= mysqli_query($koneksi, "SELECT * FROM tb_booking WHERE jadwal = '$tanggal'");
	...
	if ($cek > 1) {
		...
		if (in_array($eksetensi,$eksenti_diperbolehkan)) {
			...
			$query3 = "INSERT INTO tb_booking (id_booking,id_pelanggan,id_kendaraan,dp,jadwal,metode_pembayaran,gbr,status) VALUES 
			...
			$query_run3 = mysqli_query($koneksi,$query3) or die(mysqli_error($koneksi));
				...
				if ($query_run1 && $query_run2 && $query_run3) {
					...
					if($send){
						...
						$_SESSION['error'] = 'Pesanan Anda Gagal Terkirim';
